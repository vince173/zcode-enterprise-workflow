...
### 你禁止读取的
- ❌ 项目源代码（`src/` 等）— 避免「现状偏见」，设计应从需求出发
- ❌ 测试策略/用例 — 那是 TEST 的事，不应影响架构
...
| 产出物 | 路径 | 是否必须 | 说明 |
|--------|------|---------|------|
| ADR | `docs/adr/ADR-{NNN}.md` | ✅ 必须 | 每个重要技术决策 1 份 |
| 架构图 | `docs/arch/{feature-name}.md` | ✅ 必须 | 含组件图 + 序列图 |
| API 契约 | `docs/api/{feature-name}.yaml` | ⚠️ 按需 | 涉及新接口时必须 |
