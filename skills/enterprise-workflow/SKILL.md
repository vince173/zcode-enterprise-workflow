...
CodeOwner 阶段的产出（Code Review Report）包含对 DEV 代码的审批结论。当 CodeOwner 自身 Gate 通过但审批结论为「FAIL REJECTED」时：
...
6. **循环终止条件**：
   - CodeOwner 审批结论为「PASS」或「WARN CONDITIONAL」→ 标记 CodeOwner `completed`，推进到 TEST
   - 达到 3 轮（rc3）仍打回 → 停止循环，提示用户手动介入

```
编排器自动循环：
  DEV(rc1) → CodeOwner 审查 → FAIL REJECTED
      ↓
  DEV(rc2) ← Review Report 作为输入
      ↓
  CodeOwner 再次审查 → FAIL REJECTED
      ↓
  DEV(rc3) ← Review Report 作为输入
      ↓
  CodeOwner 再次审查 → PASS → TEST
```
...
PASS PM        v1  通过  → docs/prd/{feature}.md
PASS UX        v1  通过  → docs/ux/flow-{feature}.md
RUNNING SE        v1  运行中...
PENDING DBA            待执行
PENDING Security       待执行
PENDING DEV            待执行
PENDING CodeOwner      待执行
PENDING TEST           待执行
PENDING DevOps         待执行
...
- PASS 使用 `Agent` 工具启动角色 Agent 和 Reviewer Agent
- PASS 使用 `Write` 和 `Edit` 工具写入文件
- PASS 使用 `Read` 工具读取 state.json 和角色 SKILL.md
- FAIL 不替角色 Agent 做内容决策
- FAIL 不跳过 Gate 检查
- FAIL 不在 Gate FAIL 时强制推进
